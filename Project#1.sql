...
CREATE OR REPLACE VIEW wineriesCompanies AS
...
RIGHT JOIN companies c ON c.companies_id = wc.companies_id;
...
SELECT wc.wineries_id, wc.companies_id, c.* FROM wineries_companies wc
RIGHT JOIN companies c ON c.companies_id = wc.companies_id;

SELECT wl.*, l.* FROM wineries_location wl
RIGHT JOIN location l ON l.location_id = wl.location_id;

SELECT ws.*, s.* FROM wineries_specialty ws
RIGHT JOIN specialty s ON s.specialty_id = ws.specialty_id;
