...
/*
    ...
    Description: A winery database that keeps track of various 
                 ...
                 companies_Location
				  ...
				  NOTE: Wineries_Wine & Wineries_Tasting_Room tables are backup tables in case the relation to 
						Wineries, Wines, or Tasting_Rooms become comprimised in some way.  These are just
						in case tables so the REFERENCES and the FOREIGN KEYS can be stored if a glitch
...
select Wineries_ID, name, symbol
from Wineries
...
select wines_name, Wines_ID, Amount_made as WineTotal, Wineries.name as Name_Of_Winery
from Wines
LEFT JOIN Wineries ON Wines.Wineries_ID = Wineries.Wineries_ID
group by Wines_ID;
...
DELIMITER //
				create procedure Total_Wines(_Wines_ID int)
                 begin
					select (Name_Of_Winery), Wines_ID, wines_name, (WineTotal) from Amount_Of_Wines where Wines_ID = _Wines_ID;
				 end //
